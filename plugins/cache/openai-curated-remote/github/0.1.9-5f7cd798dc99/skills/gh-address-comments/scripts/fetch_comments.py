...
def get_current_pr_ref() -> tuple[str, str, int]:
    ...
    Resolve fork PRs against their base repository, which owns the PR number.
    """
    pr = gh_pr_view_json("number,url")
    number = int(pr["number"])
    parts = [part for part in urlparse(pr["url"]).path.split("/") if part]
    if len(parts) < 4 or parts[2] != "pull" or parts[3] != str(number):
        raise RuntimeError(f"Unable to determine the PR base repository from {pr['url']!r}")
    owner, repo = parts[:2]
...
def fetch_all(owner: str, repo: str, number: int) -> dict[str, Any]:
    ...
    while True:
        ...
        if not comments_complete:
            ...
            comments_complete = not c["pageInfo"]["hasNextPage"]
            comments_cursor = c["pageInfo"]["endCursor"] if not comments_complete else None
        if not reviews_complete:
            reviews.extend(r.get("nodes") or [])
            reviews_complete = not r["pageInfo"]["hasNextPage"]
            reviews_cursor = r["pageInfo"]["endCursor"] if not reviews_complete else None
        if not threads_complete:
            review_threads.extend(t.get("nodes") or [])
            threads_complete = not t["pageInfo"]["hasNextPage"]
            threads_cursor = t["pageInfo"]["endCursor"] if not threads_complete else None

        if comments_complete and reviews_complete and threads_complete:
            break
